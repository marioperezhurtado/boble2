...
export function getMessages(chatId: string) {
  return (
    db
      ...
      .leftJoin(
        linkPreview,
        and(eq(message.type, "link"), eq(message.source, linkPreview.url)),
      )
      // join document info to document messages
      .leftJoin(
        documentInfo,
        and(eq(message.type, "document"), eq(message.source, documentInfo.url)),
      )
  );
}

export type Messages = typeof getMessages extends (
  ...args: any
) => Promise<infer T>
  ? T
  : never;
