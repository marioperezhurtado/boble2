...
export function ChatList() {
    ...
    return (<section class="w-full max-w-sm bg-zinc-50">
      <div class="flex gap-2 p-2">
        <input id="message" name="message" type="search" placeholder="Search chats..." class="py-1.5 px-2 w-full text-sm rounded-md border shadow-sm placeholder:text-zinc-400 focus:outline-cyan-600" autocomplete="off"/>
        <button type="button" title="Sort chats" aria-label="Sort chats" class="rounded-md focus:outline-cyan-600">
      ...
      <ul class="border-t">
        <For each={DUMMY_CHATS}>
          {(chat) => (<li>
              <A href={`/${chat.id}`} class="flex gap-3 p-2 border-b border-r-[3px] border-r-transparent" classList={{
                ...
                <div class="flex flex-col flex-1 gap-0.5">
                  ...
                  <p class="w-64 text-sm text-zinc-600 truncate" title={chat.lastMessage.text}>
